...
activityRoutes.get("/activity/:activityId/participants", authenticateJWT, async (req, res) => {
  ...
  if (mongoose.Types.ObjectId.isValid(activityId)) {
    try {
      ...
      ).populate("participants", "id first_name last_name email");
      if (!activity) {
...
activityRoutes.patch("/activity/:activityId", authenticateJWT, async (req, res) => {
  if (denyChangeRequests === "true") {
    ...
    if (mongoose.Types.ObjectId.isValid(activityId)) {
      try {
        const updated = await Activity.findOneAndUpdate({ _id: activityId, "organizers._id": accountId }, updatedActivity, {
          new: true,
          runValidators: true,
        });
        if (!updated) {
